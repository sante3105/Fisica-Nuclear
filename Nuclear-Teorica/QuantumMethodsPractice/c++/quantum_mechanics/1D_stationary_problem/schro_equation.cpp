...
static double v(double x) {
    ...
    return 0.25 * x * x;  // Uncommented to use the harmonic oscillator
}
...
int main(void) {
    ...
    for (size_t j = 0; j < N; ++j) {
        double norm = 0.0;
        for (size_t i = 0; i < N; ++i) {
            double val = gsl_matrix_get(evec, i, j);
            norm += val * val;
        }
        norm = sqrt(norm * d);  // Include grid spacing in normalization
        
        // Normalize the eigenvector
        for (size_t i = 0; i < N; ++i) {
            double val = gsl_matrix_get(evec, i, j);
            gsl_matrix_set(evec, i, j, val / norm);
        }
    }
    ...
    for (size_t j = 0; j < 4 && j < N; ++j) {
        ...
        // Calculate the sign of the wavefunction to ensure ground state is positive
        // at its maximum (convention)
        double max_psi = 0.0;
        size_t max_idx = 0;
        for (size_t i = 0; i < N; ++i) {
            double psi_val = fabs(gsl_matrix_get(evec, i, j));
            if (psi_val > max_psi) {
                max_psi = psi_val;
                max_idx = i;
            }
        }
        double sign_factor = (gsl_matrix_get(evec, max_idx, j) >= 0) ? 1.0 : -1.0;
        ...
        for (size_t i_full = 0; i_full < Nfull; ++i_full) {
            ...
            if (i_full > 0 && i_full < Nfull-1) {
                size_t matrix_idx = i_full - 1;  // Corrected index mapping
                psi = sign_factor * gsl_matrix_get(evec, matrix_idx, j);
            }
            fprintf(fp, "%g %g %g\n", x, psi, psi*psi);
        }
        fclose(fp);
